...
**選擇題格式規範**：
- 四選一，選項標籤在生成時必須呈現 ➀、➁、➂、➃，不得使用 A/B/C/D 或 a/b/c/d 作為選項標記。
- JSON 中的 `options` 仍然使用 `"A"`、`"B"`、`"C"`、`"D"` 作為鍵值（對應 ➀、➁、➂、➃）以供後端腳本正確轉換。
- 干擾選項需具學習意義（常規計算錯誤或概念迷思）。
- 每題明確有唯一正確答案。

**答案分佈規則（必須嚴格遵守）**：
- 答案必須平均分佈於 A、B、C、D 四個選項，每個選項出現次數應大致相等（±1題以內）。
- **禁止連續兩題答案相同**（例如：第3題答 A，第4題不得再答 A）。
...
- **未知數（重要限制 ⚠️）**：國小中年級與高年級（一至五年級）試卷中，為了避免家長誤會超出範圍，**禁止在題目文字中直接出現英文字母如 `x` 或 `y`**。
  - 未知數一律使用 `□`（方框）或 `( )`（括號）呈現（例如：`7 × □ = 672`）。
  - 若在 JSON 資料庫中寫入 `{x}` 或 `{y}`，後端編譯器在產生 Word 時會自動將其替換為空的「□」方框。
- **應用題子題標號規範**：
  - 如果計算題或應用題**只有一個問題，禁止使用 `(1)` 標籤進行編號**，必須直接將問題寫在情境敘述之後或作為單一自然段呈現。
  - 只有在該大題有「多個連續子問題」時，才允許使用 `(1)`、`(2)` 等標籤。
- **四則運算與不等式**：
  - 乘號：使用 `{a*b}` ➔ 轉為 `a×b`
  - 關係符號：使用 `{<=}` (≤)、`{>=}` (≥)、`{!=}` (≠)
