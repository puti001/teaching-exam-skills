...
- **未知數（重要限制 ⚠️）**：國小（一至五年級）試卷中，為了避免家長認為超出範圍，**禁止直接出現字母變數 `x` 或 `y`**。未知數一律使用 `□` 或 `( )` 呈現。若在後台寫入 `{x}` 或 `{y}`，編譯腳本會自動將其在 Word 中渲染為 `□`（方框）。
- **應用題標號限制**：如果題目只有一個子問題，請直接將問題敘述接在題幹後或單一自然段呈現，**禁止標上 `(1)` 標籤**。只有在有兩個或多個子問題時才允許標上 `(1)`、`(2)` 等標號。
- **不等式與四則運算**：使用 `{<=}` (≤)、`{>=}` (≥)、`{!=}` (≠)、`{*}` (×)。
